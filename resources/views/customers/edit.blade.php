...
$(document).ready(function() {
    ...
    const iti = window.intlTelInput(input, {
        initialCountry: "{{ Helper::getIso2ByDialCode($customer->dial_code) }}",
    ...
    @if(isset($customer->country->id))
        $('#country_id').append(new Option('{{ $customer->country->name ?? '' }}', '{{ $customer->country->id ?? '' }}', true, true));
    @endif
    @if(isset($customer->state->id))
        $('#state_id').append(new Option('{{ $customer->state->name ?? '' }}', '{{ $customer->state->id ?? '' }}', true, true));
    @endif
    @if(isset($customer->city->id))
        $('#city_id').append(new Option('{{ $customer->city->name ?? '' }}', '{{ $customer->city->id ?? '' }}', true, true));
    @endif

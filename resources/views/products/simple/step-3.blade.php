...
<div class="row">
    <div class="col-12">
        <div id="pricing-matrix" class="mt-4">
            ...
            <p>Set quantity-based pricing for individual <span class="titleOfCurrentTabUnit">{{ $baseUnit?->unit?->title ?? 'N/A' }}</span></p>
            ...
            <ul class="nav nav-tabs" role="tablist">
                @if($baseUnit)
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link active" id="{{ $baseTabId }}-tab" data-current-unit="{{ $baseUnit->unit->title ?? 'N/A' }}" data-bs-toggle="tab" href="#{{ $baseTabId }}" role="tab" aria-controls="{{ $baseTabId }}" aria-selected="true">{{ $baseUnit->unit->title ?? 'N/A' }} (Base Unit)</a>
                    </li>
                @endif
                @foreach($additionalUnits as $row)
                    ...
                    @endphp
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" id="{{ $tabId }}-tab" data-current-unit="{{ $row->unit->title ?? 'N/A' }}" data-bs-toggle="tab" href="#{{ $tabId }}" role="tab" aria-controls="{{ $tabId }}" aria-selected="false">{{ $row->unit->title ?? 'N/A' }}</a>
                    </li>
                @endforeach
            </ul>
            ...
            <div class="tab-content">
                @if($baseUnit)
                    @php
                        $baseTabId = md5('base-'.$baseUnit->id);
                    @endphp
                    <div class="tab-pane fade show active" id="{{ $baseTabId }}" role="tabpanel" aria-labelledby="{{ $baseTabId }}-tab">
                        <table class="table table-bordered pricing-table-instance" data-variant-id="" data-unit-row-id="{{ $baseUnit->id }}">
                            <thead>
                                <tr>
                                    <th>Min Quantity</th>
                                    <th>Max Quantity</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                    <input type="hidden" name="is_base_unit" class="is_base_unit" value="1">
                                    <input type="number" class="form-control" name="min_quantity[]" value="{{ $tier->min_qty }}" min="1" step="1"></td>
                ...
                @foreach($additionalUnits as $row)
                    @php
                        $tabId = md5('add-'.$row->id);
                    @endphp
                    <div class="tab-pane fade @if(!$baseUnit && $loop->first) show active @endif" id="{{ $tabId }}" role="tabpanel" aria-labelledby="{{ $tabId }}-tab">
                        <table class="table table-bordered pricing-table-instance" data-variant-id="" data-unit-row-id="{{ $row->id }}">
                            <thead>
                                <tr>
                                    <th>Min Quantity</th>
                                    <th>Max Quantity</th>
                ...
                $(this).find('tbody tr').each(function() {
                    ...
                    if (!isNaN(minQty) || !isNaN(maxQty) || !isNaN(price) || !isNaN(discount)) {
                        items.push({
                            ...
                            is_base_unit: $(this).find('.is_base_unit').length ? 1 : 0,

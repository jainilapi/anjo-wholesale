...
// Get additional categories (flat list for checkboxes)
$additionalCategories = \App\Models\Category::whereNull('deleted_at')
    ->where('status', 1)
    ->orderBy('name')
    ->get();

// Get selected categories if product exists
...
$(document).ready(function() {
    ...
    // Ensure only one primary category is selected
    ...
    // Toggle category tree expand/collapse
    ...
    // Prevent selecting primary category as additional
    $('input[name="primary_category"]').on('change', function() {
        if ($(this).is(':checked')) {
